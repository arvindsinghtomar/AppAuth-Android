...
public class TokenActivity extends AppCompatActivity {
    ...
    private void receivedTokenResponse(
        ...
        if (this.mTokenResponse != null
                && this.mTokenResponse.idToken != null
    ...
    private void performTokenRequest(TokenRequest request) {
        ...
        try {
            ...
            if ((mAuthState.getClientSecret() == null
                    ...
                    && IdentityProvider.getEnabledProviders(TokenActivity.this)
                    .get(0).getClientSecret() != null)) {
                clientAuthentication = new ClientSecretBasic(IdentityProvider
                        .getEnabledProviders(TokenActivity.this)
                        .get(0).getClientSecret());
            }
    ...
    private void performTokenValidation(TokenResponse response) {
        ...
        try {
            clientAuthentication = mAuthState.getClientAuthentication();
            if ((TextUtils.isEmpty(mAuthState.getClientSecret())
                    && IdentityProvider.getEnabledProviders(TokenActivity.this)
                    .get(0).getClientSecret() != null)) {
                clientAuthentication = new ClientSecretBasic(IdentityProvider
                        .getEnabledProviders(TokenActivity.this).get(0).getClientSecret());
            }
        } catch (ClientAuthentication.UnsupportedAuthenticationMethod ex) {
            Log.d(TAG, "Token request cannot be made, client authentication for the token "
                    + "endpoint could not be constructed (%s)", ex);
        ...
        mAuthService.performTokenValidation(
                ...
                new AuthorizationService.TokenValidationResponseCallback() {
                    @Override
                    public void onTokenValidationRequestCompleted(boolean isTokenValid,
                                                                  @Nullable AuthorizationException
                                                                          ex) {
                        if (isTokenValid) {
                            mAuthState.update(mTokenResponse, ex);
                            showSnackbar((mTokenResponse != null)
                                    ? R.string.exchange_complete
                                    : R.string.refresh_failed);
                            refreshUi();
                        } else {
                            mAuthState.update(mTokenResponse, ex);
                            showSnackbar(R.string.refresh_failed);
                        }
                    }
                });
    }
